  ...
  const save = async () => {
    if (username && mobile && email && password && rePassword && dob && name && address) {
      ...
      if (a === "ok" && b === "ok" & c === "ok" && msgAlert()) {
        let link = "http://localhost:8080/admin/create-admin"
        const response = await fetch(link, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ username: username, dob: dob, name: name,batchId:batchId,station:assignedDistrict, mobile: mobile, email: email, password: rePassword, address: address })
        });
        const result = await response.text()
        if (result === "saved") {
          return result
  ...
  return (
    <div className="flex justify-center py-12 bg-[#111C30] text-white border-t-white border-t-2">
      <div className=" h-full p-20 w-4/7 bg-[#0B1524] rounded-2xl ">
        ...
        <h1 className="font-bold text-4xl pb-4 text-center">Add Admin</h1>
        ...
        <form onSubmit={(e) => {
          ...
          <div className="flex gap-30 mt-5">
            ...
            <div>
              ...
              />
              <label className="block  text-sm font-bold mb-2" htmlFor="AssignDistrict">
               Assign District
              </label>
              <input
                required
                id="AssignDistrict"
                type="text"
                placeholder="Enter your Assign District"
                className="w-full bg-gray-200 text-gray-800 pl-2 mr-30  py-3 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-300 placeholder-gray-500 text-sm"
