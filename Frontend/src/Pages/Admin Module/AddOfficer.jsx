...
const AddOfficer = () => {
  ...
  const save = async () => {
    if (username && mobile && email && password && rePassword && dob && name && address) {
      ...
      if (a === "ok" && b === "ok" & c === "ok" && msgAlert()) {
        let link = "http://localhost:8080/admin/create-new-police"
        const response = await fetch(link, {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({ username: username, dob: dob, name: name, mobile: mobile, batchId: batchId, station: assignedStation, email: email, password: rePassword, address: address })
        });
        const result = await response.text()
        if (result === "saved") {
          return result
  ...
  return (
    <div className="flex justify-center py-12 bg-[#111C30] text-white border-t-white border-t-2">
      <div className=" h-full p-20 w-4/7 bg-[#0B1524] rounded-2xl ">

        <h1 className="font-bold text-4xl pb-4 text-center">Add Officer</h1>
        ...
        <form onSubmit={(e) => {
          ...
          <div className="flex gap-30 mt-5">
            <div>
              ...
              />
              <div className='flex flex-row justify-between mr-3'>
                <label className="block font-bold   mb-2" htmlFor="BatchId">
                  Batch Id
                </label>
              </div>
              <input
            ...
            <div>
              ...
              />
              <label className="block  text-sm font-bold mb-2" htmlFor="AssignStation">
                Assigned Station
              </label>
              <input
                required
                id="AssignedStation"
                type="text"
                placeholder="Enter your Assign Station"
                className="w-full bg-gray-200 text-gray-800 pl-2 mr-30  py-3 rounded-2xl focus:outline-none focus:ring-2 focus:ring-blue-300 placeholder-gray-500 text-sm"
                value={assignedStation}
                onChange={(e) => {
                  setAssignedStation(e.target.value);
                }}
              />
